...
float imgStrategic(ImgFunType funtype, IplImage* imgprev, IplImage* imgcurr, IplImage* imgdst, Mat &color, Mat &gray, int strategic){
	//计算光流初始化变量
	IplImage* imgprev_1 = imgResize(imgprev);
	IplImage* imgcurr_1 = imgResize(imgcurr);
	CvMat *velx, *vely;
	...
	if ((strategic>> 5 & 1) == 1) //motion to gray
	{
		...
		if (IS_SIMILARITY)
		{
			...
			IplImage* prev_1 = cvCloneImage(imgprev_1);
...
float matStrategic(MatFunType funtype, Mat frameprev, Mat framecurr, Mat &framedst, Mat &color, Mat &gray, int strategic,bool issf){
    Mat frameprev_1, framecurr_1, flow,roi,true_roi;
	if (issf)
	{
    ...
    if ((strategic>> 6 & 1) == 1 && (strategic>> 5 & 1) == 1) //draw flow without zero
    {

        roi = Mat(gray,Range(0,HEIGHT/2),Range::all());

        vector<vector<Point> > contours_true,contours_flow;//轮廓
        vector<Vec4i> hierarchy_true,hierarchy_flow;//层次结构
        Mat framecurr_32f,gray_32f;
        framecurr_1.convertTo(framecurr_32f,CV_32F);
        gray.convertTo(gray_32f,CV_32F);



        buaaMatchShapes(frameprev_1,contours_true,hierarchy_true,"real contours");
        buaaMatchShapes(gray,contours_flow,hierarchy_flow,"flow contours");
        cout << gray_32f.checkVector(2) << endl << framecurr_32f.checkVector(2);



        double similarity_index = matchShapes(gray_32f,framecurr_32f,CV_CONTOURS_MATCH_I1,0);
        ofstream out;\
        out.open("text.txt",ios_base::app);
        out << similarity_index << endl;
        out.close();
